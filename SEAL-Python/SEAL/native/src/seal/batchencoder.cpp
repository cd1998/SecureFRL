...
namespace seal
{
    ...
    void BatchEncoder::encode(const vector<uint64_t> &values_matrix, Plaintext &destination) const
    {
        ...
        if(encmd == encoding_method::crt) {
            std::cout << "default encoding method1.\n";
        ...
        } else if (encmd == encoding_method::pm) {
            std::cout << "PM encoding method1.\n";
            for (std::size_t i = 0; i < values_matrix_size; i++) {
            	destination[i] = values_matrix[i];
            }
        }
    }
    ...
    void BatchEncoder::encode(const vector<int64_t> &values_matrix, Plaintext &destination) const
    {
        ...
        destination.parms_id() = parms_id_zero;
        if(encmd == encoding_method::crt) {
            std::cout << "default encoding method2.\n";
            // First write the values to destination coefficients.
            // Read in top row, then bottom row.
            for (size_t i = 0; i < values_matrix_size; i++)
            {
            ...
            std::cout << "PM encoding method2.\n";
            for (std::size_t i = 0; i < values_matrix_size; i++) {
            	destination[i] = (values_matrix[i] < 0) ? (modulus + static_cast<uint64_t>(values_matrix[i]))
                                        : static_cast<uint64_t>(values_matrix[i]);
            }
        }
    ...
    void BatchEncoder::decode(const Plaintext &plain, vector<uint64_t> &destination, MemoryPoolHandle pool) const
    {
        ...
        if (encmd == encoding_method::crt){
            std::cout << "default decoding method1.\n";
            ...
            std::cout << "PM decoding method1.\n";
            // 直接复制多项式系数到目标向量
            for (size_t i = 0; i < slots_; i++)
            {
                destination[i] = plain.data()[i];
            }
        }
    }
    ...
    void BatchEncoder::decode(const Plaintext &plain, vector<int64_t> &destination, MemoryPoolHandle pool) const
    {
        ...
        if (encmd == encoding_method::crt){
            std::cout << "default decoding method2.\n";
            auto temp_dest(allocate_uint(slots_, pool));
            ...
            uint64_t plain_modulus_div_two = modulus >> 1;
            std::cout << "PM decoding method2.\n";
            for (size_t i = 0; i < slots_; i++)
            {
                uint64_t value = plain.data()[i];
                destination[i] = (value > plain_modulus_div_two) ? static_cast<int64_t>(value) - static_cast<int64_t>(modulus) : static_cast<int64_t>(value);
            }
        }
