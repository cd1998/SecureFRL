...
namespace seal
{
    ...
    void EncryptionParameters::save_members(ostream &stream) const
    {
        ...
        try
        {
            ...
            uint8_t encmd = static_cast<uint8_t>(encmd_);

            stream.write(reinterpret_cast<const char *>(&scheme), sizeof(uint8_t));
            stream.write(reinterpret_cast<const char *>(&encmd), sizeof(uint8_t));
    ...
    void EncryptionParameters::load_members(istream &stream, SEAL_MAYBE_UNUSED SEALVersion version)
    {
        ...
        try
        {
            ...
            uint8_t encmd;
            stream.read(reinterpret_cast<char *>(&encmd), sizeof(uint8_t));
            cout << "encmd = " << encmd << endl;

            // This constructor will throw if scheme is invalid
            EncryptionParameters parms(scheme);
            parms.set_encoding_method(static_cast<encoding_method>(encmd));
    ...
    void EncryptionParameters::compute_parms_id()
    {
        ...
        *param_data_ptr++ = static_cast<uint64_t>(encmd_);
