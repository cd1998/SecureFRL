    ...
    */
    ...
    public:
        ...
        */
            ...
            const Plaintext &plain, std::vector<T> &destination, MemoryPoolHandle pool = MemoryManager::GetPool()) const
        {
            ...
            if (parms.encodmd() == encoding_method::crt)
            	destination.resize(slots_);
            else destination.resize(2 * slots_);
        ...
        */
            ...
            const Plaintext &plain, gsl::span<T> destination, MemoryPoolHandle pool = MemoryManager::GetPool()) const
        {
            ...
            if ((destination.size() != slots_ && encmd == encoding_method::crt) || destination.size() != slots_ * 2)
            {
    ...
    private:
	    ...
	    auto &context_data = *context_data_ptr;
            ...
            if (encmd == encoding_method::crt && values_size > slots_)
            {
            ...
            if(encmd == encoding_method::crt) {
            	std::cout << "default encoding method.\n";
            ...
            } else if (encmd == encoding_method::pm) {		// pm1
            	std::cout << "PM encoding method.\n";
            	for (std::size_t i = 0; i < values_size; i++) {
            		conj_values[i] = values[i] * scale;
            	}
            } else {				// pm2
            	std::cout << "PM2 encoding method.\n";
            	for (std::size_t i = 0; i < values_size; i++) {
            		conj_values[i] = values[values_size - i - 1] * scale;
            	}
            }
        ...
        void decode_internal(const Plaintext &plain, T *destination, MemoryPoolHandle pool) const
        {
            ...
            if(encmd == encoding_method::crt) {
            	std::cout << "default decoding method.\n";
            	...
            	std::cout << "PM decoding method.\n";
            	std::size_t n = util::mul_safe(slots_, std::size_t(2));
            	for (std::size_t i = 0; i < n; i++)
            		destination[i] = from_complex<T>(res[i]);
            } else {
            	std::cout << "PM2 decoding method.\n";
            	std::size_t n = util::mul_safe(slots_, std::size_t(2));
            	for (std::size_t i = 0; i < n; i++)
            		destination[i] = from_complex<T>(res[n - i - 1]);
            }
        }
